...
from app.transform.transform import DMIDataTransformer, SpecDataTransformer
...
class ETLProcess:
    ...
    def update_database(self):
        ...
        try:
            ...
            raise RuntimeError(
                f"Required file missing: {stations_file} in path {file_path}"
            )
        ...
        for station in stations:
            for parameterId in params:
                self.dmi_etl(
                    stations[station],
                    parameterId,
                    from_time=times_dict["DMI"][parameterId],
                )
    ...
    def dmi_etl(
        ...
        station_id,
        parameter_id,
        from_time: str = "2026-03-09T00:00:00Z",
        max_pulls: int = None,
        limit: int = 5000,
    ):
        api = DMIAPI()
        ...
        print(
            f"Pulling {parameter_id} data with stationid {station_id} from the DMI API"
        )
        while True:
            pull_time, records = api.pull_datetime(
            ...
            from_time = self.advance_timestamp(
                max(r["properties"]["observed"] for r in records)
            )
            ...
            print(
                f"{limit} records pulled. Exporting pull times to json. Elapsed time: {elapsed_time}"
            )
    ...
    def spec_etl(
        ...
        while True:
            ...
            from_time = self.advance_timestamp(
                min(last_bme["timestamp"], last_ds["timestamp"])
            )
            ...
            for table_name in db_dict:
                self.crud.create_mult_rows(
                    table_name, db_dict[table_name], commit=True, close=False
                )

            elapsed_time = time.time() - start_time
            print(
                f"5000 records pulled. Exporting pull times to json. Elapsed time: {elapsed_time}"
            )
    ...
    def check_max_vs_total_pulls(self, max_pulls, total_pulls, start_time):
        if max_pulls is not None and total_pulls >= max_pulls:
            elapsed_time = time.time() - start_time
            print(
                f"Reached maximum number of pulls. Aborting ETL. Elapsed time: {elapsed_time}"
            )
            return True
        return False
    ...
    def export_start_times(self, from_time, api: str, parameter_id=None):
        ...
        if api == "DMI":
            ...
            raise ValueError(
                "Incorrect API name given. The API must be 'DMI' or 'spec'."
            )
    ...
    def start_etl_background(self, interval_minutes: int = 10):
        ...
        thread = threading.Thread(
            target=self.run_etl_periodically, args=(interval_seconds,), daemon=True
        )
        thread.start()
    ...
    def new_spec_etl(
        self,
        from_time: str = "2026-03-17T00:00:00Z",
        max_pulls: int = None,
        limit: int = 5000,
    ):
        api = NewSpecAPI()
        ...
        while True:
            ...
            from_time = self.advance_timestamp(
                min([last_readings[key]["timestamp"] for key in last_readings])
            )
    ...
    def get_last_readings(self, records):
        ...
        for r in records:
            if (
                "BME280" in r["reading"]
                and r["location"]["value"] == "00000000adae116e-percepter-ballerup-in"
            ):
                last_readings["last_bme_in"] = r
            elif (
                "BME280" in r["reading"]
                and r["location"]["value"] == "000000005b900eb3-percepter-ballerup-out"
            ):
                last_readings["last_bme_out"] = r
            elif (
                "DS18B20" in r["reading"]
                and r["location"]["value"] == "00000000adae116e-percepter-ballerup-in"
            ):
